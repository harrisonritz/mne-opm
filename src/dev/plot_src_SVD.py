...
print(
    f"Number of dipoles - LH: {n_dipoles_lh}, RH: {n_dipoles_rh}, Total: {n_dipoles_total}"
)
...
def plot_singular_vector(
    mode_idx, mode_data_lh, mode_data_rh, vertices, subject, subjects_dir
):
    ...
    stc = mne.SourceEstimate(
        ...
        data=np.concatenate((mode_data_lh, mode_data_rh))[
            :, np.newaxis
        ],  # (n_dipoles_total, 1)
        vertices=vertices,
        tmin=0,
...
for i in range(n_modes):
    ...
    brain = plot_singular_vector(
        i, mode_magnitudes_lh[i], mode_magnitudes_rh[i], vertices, subject, subjects_dir
    )
    brains.append(brain)
...
ax.axvline(
    np.argmax(sv_pct >= 95) + 1,
    color="blue",
    linestyle="--",
    label=f"95% variance at mode {np.argmax(sv_pct >= 95) + 1}",
)
ax.axvline(
    np.argmax(sv_pct >= 99) + 1,
    color="darkblue",
    linestyle="--",
    label=f"99% variance at mode {np.argmax(sv_pct >= 99) + 1}",
)
