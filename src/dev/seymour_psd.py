...
def mark_bad_var(raw_downsampled):
    ...
    time_points = np.arange(0, (num_chunks) * (chunk_size / raw_downsampled.info['sfreq']), chunk_size / raw_downsampled.info['sfreq'])
    ...
    mean_line = ax.axhline(mean_var, color='green', linestyle='--',linewidth=1, label='Median Variance')
    threshold_line, = ax.plot([time_points[0], time_points[-1]], [initial_threshold, initial_threshold], 'r--', lw=2, label='Threshold')
    ...
    threshold_slider = Slider(ax_slider, 'Threshold', np.min(chunk_variances), np.max(chunk_variances), valinit=initial_threshold)
    ...
    ax_checkbox = plt.axes([0.4, 0.05, 0.3, 0.1])  # Larger checkbox, no yellow background, centered
    ...
    def on_apply(label):
        ...
        if apply_checkbox.get_status()[0]:
            ...
            bad_segment_indices = [idx for idx, anno in enumerate(existing_annotations) if anno['description'] == 'Bad Segment']
            ...
            combined_onset = np.concatenate([existing_annotations.onset, new_annotations.onset])
            combined_duration = np.concatenate([existing_annotations.duration, new_annotations.duration])
            combined_description = np.concatenate([existing_annotations.description, new_annotations.description])

            # Set the combined annotations
            raw_downsampled.set_annotations(mne.Annotations(
...
def detect_bad_chan_PSD(raw_downsampled, alpha=0.05, fmin=2, fmax=80, verbose=True,n_fft=2000):
    """
    ...
    # Compute PSD
    psd = raw_downsampled.compute_psd(fmin=fmin, fmax=fmax, n_fft=n_fft, reject_by_annotation=True, verbose=True)
    pow = psd.get_data()
    freq = psd.freqs
    ...
    def compute_bad_channels(alpha):
        ...
        # Detect bad channels
        bad_channels_mask = detect_artefacts(pow_log, axis=0, reject_mode='dim', gesd_args=gesd_args)
    ...
    lines = [ax.semilogy(freq, data, alpha=0.1, label=chan, color='blue')[0] for data, chan in zip(pow, chan_names)]
